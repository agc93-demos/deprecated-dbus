...
namespace UPower.DBus
{
    ...
    interface IKbdBacklight : IDBusObject
    {
        Task<int> GetMaxBrightnessAsync();
        Task<int> GetBrightnessAsync();
        Task SetBrightnessAsync(int Value);
        Task<IDisposable> WatchBrightnessChangedAsync(Action<int> handler, Action<Exception> onError = null);
        Task<IDisposable> WatchBrightnessChangedWithSourceAsync(Action<(int value, string source)> handler, Action<Exception> onError = null);
    }
